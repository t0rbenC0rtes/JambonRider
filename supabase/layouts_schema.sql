...
-- Anyone can insert layouts (change to authenticated if needed)
CREATE POLICY "Public can insert layouts"
ON layouts FOR INSERT
WITH CHECK (true);

-- Anyone can update layouts (change to authenticated if needed)
CREATE POLICY "Public can update layouts"
ON layouts FOR UPDATE
USING (true);

-- Anyone can delete layouts (change to authenticated if needed)
